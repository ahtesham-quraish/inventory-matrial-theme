...
class Receivable extends React.Component {
  ...
  render() {
    ...
    let tableData = { readyData: [], Belence: 0 };
    if (customerType === 'Buyer') {
      tableData = prepareReceivableTableData(transactions, dateFilter);
    }
    if (this.props.customerType === 'Supplier') {
      tableData = preparePayableTableData(transactions, dateFilter);
    }
    ...
    return (
      ...
      >
        <Card style={{ width: '500px' }} className={'blue'}>
          <CardHeader color="primary">
            ...
            <h6 className={classes.cardTitleWhite}>
              {customerType} with balances at {this.state.dateFilter}
            </h6>
          </CardHeader>
          <CardBody>
            ...
            <Table
              ...
              tableHead={['Customer', 'Belence']}
              tableData={tableData.readyData}
...
const styledCompoenet = withStyles(styles)(Receivable);
